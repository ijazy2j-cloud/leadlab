...
**Non-obvious**: Light text on dark backgrounds needs compensation on three axes, not just one. Bump line-height by 0.05â€“0.1, add a touch of letter-spacing (0.01â€“0.02em), and optionally step the body weight up one notch (regular â†’ medium). The perceived weight drops across all three; fix all three.
...
**Variable fonts for 3+ weights or styles**: a single variable font file is usually smaller than three static weight files, gives fractional weight control, and pairs well with `font-optical-sizing: auto`. For 1â€“2 weights, static is fine.
...
**Bound your clamp()**: keep `max-size â‰¤ ~2.5 Ã— min-size`. Wider ratios break the browser's zoom and reflow behaviour and make large viewports feel like the page is shouting.

**Scale container width and font-size together** so effective character measure stays in the 45â€“75ch band at every viewport. A heading that widens faster than its container drifts out of the comfortable measure at the top end.
...
**ALL-CAPS tracking**: capitals sit too close at default spacing. Add 5â€“12% letter-spacing (`letter-spacing: 0.05em` to `0.12em`) to short all-caps labels, eyebrows, and small headings. Real small caps (via `font-variant-caps`) need the same treatment, slightly gentler.
