 ...
 * uses Server-Sent Events (SSE) for serverâ†’browser push, and HTTP POST for
 * browserâ†’server events. Agent communicates via HTTP long-poll (/poll).
 *
 * Usage:
...
const DEFAULT_POLL_TIMEOUT = 600_000;   // 10 min â€” agent re-polls on timeout anyway
...
const state = {
  token: null,
  port: null,
  sseClients: new Set(),   // SSE response objects (serverâ†’browser push)
...
// Cap per-annotation upload size. A full 1920Ã—1080 PNG is typically <1 MB;
...
function loadBrowserScripts() {
  ...
  // This one IS cached â€” detect.js rarely changes during a session.
  const detectPaths = [
...
function hasProjectContext() {
  // PRODUCT.md carries brand voice / anti-references â€” that's what determines
...
// ---------------------------------------------------------------------------
// Validation (inline â€” no external import needed for self-contained script)
// ---------------------------------------------------------------------------
...
function createRequestHandler({ detectScript, sessionPath, livePath }) {
  return (req, res) => {
    ...
    if (p === '/live.js') {
      ...
      // sessions â€” during iteration, a cached old script silently breaks
    ...
    // --- Annotation upload (browser â†’ server, raw PNG body) ---
    ...
    // --- SSE: serverâ†’browser push (replaces WebSocket) ---
    ...
    // --- Browserâ†’server events (replaces WebSocket messages) ---
...
Endpoints:
  ...
  /events              SSE stream (serverâ†’browser) + POST (browserâ†’server)
