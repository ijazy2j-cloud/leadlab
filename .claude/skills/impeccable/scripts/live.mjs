 ...
 *   - Open the project's live dev/preview URL in the browser (optional, if browser automation exists)â€”not `serverPort`; that port is the Impeccable helper for /live.js and /poll
...
The agent should then:
  ...
  2. Optionally open the project's dev/preview URL in the browser (see reference/live.mdâ€”not serverPort)
  3. Enter the poll loop: node live-poll.mjs`);
    process.exit(0);
  }

  // 1. Check config (fail fast if missing â€” no point starting anything else)
  ...
  //    Warning only â€” the agent decides whether to act.
 ...
 * advisory â€” the agent can ignore it, or suggest the user add the
  ...
  try {
    ...
    if (existing && existing.pid) {
      try {
        ...
        return existing;
      } catch { /* stale PID file â€” the server script will clean it up */ }
    }
  } catch { /* no PID file */ }
