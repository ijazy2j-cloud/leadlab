...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  // not on `block.start` â€” for JSX that's the marker comment 2 spaces
  ...
  //
  // Style attribute syntax has to follow the host file's flavor â€” JSX files
 ...
 * element's JSX slot keeps a single child â€” a Fragment `<></>` would have
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Match either `<div â€¦ />` (self-close, group 1 is `/`), `<div â€¦ >`
 ...
 *   - Same-line `<style>â€¦</style>` blocks
 *   - Multi-line `<style>\nâ€¦\n</style>` blocks
 */
...
/**
 * Find the inner content of `<TAG ...attrMatch...>â€¦</TAG>` inside `text`,
...
function extractInnerByAttr(text, attrMatch) {
  ...
  // (Does not match self-closing <TAG â€¦ />, which doesn't contribute to depth.)
 ...
 *   1. Self-closing: `<style ... />` â€” no body; return null (nothing to carbonize).
 *   2. Same-line open+close: `<style>...</style>` â€” return the inner content.
 *   3. Multi-line: `<style>` on one line, `</style>` on a later line â€” return
...
function extractCss(lines, block, id) {
  ...
  for (let i = block.start; i <= block.end; i++) {
    ...
    if (inStyle) {
      // Detect </style> anywhere on the line â€” JSX template-literal closes
...
/**
 * Strip a JSX template-literal wrap (`{` â€¦ `}`) from CSS extracted out of a
 ...
 * or attached to the first/last CSS lines â€” all three are JSX-legal.
 *
 ...
 * `{` `{` â€¦ `}` `}`, which oxc rejects with "Expected `}` but found `@`".
 */
