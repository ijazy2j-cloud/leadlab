 ...
 * concrete artifact before calling this â€” we never slug a natural-language
 * phrase.
 */
...
function kebab(s) {
  const slug = s
    ...
    .replace(/^-|-$/g, '');
  if (!slug) return null;
  // Cap from the tail â€” the tail (filename) is more identifying than the
  // top-level directory.
...
/**
 * Return all snapshot files for `slug`, sorted oldest â†’ newest.
 */
...
/**
 * Return the last `limit` snapshots' frontmatter, oldest â†’ newest.
