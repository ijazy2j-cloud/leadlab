...
// 2-space convention. No arrays, no anchors, no multi-line scalars â€” Stitch's
...
function splitSections(md) {
  ...
  for (const raw of lines) {
    ...
    if (h2) {
      ...
      // non-canonical H2 â€” ignore but stop feeding into current
      current = null;
      continue;
    }
...
function extractNamedRules(lines) {
  ...
  // Style A (Impeccable): "**The X Rule.** body body body" â€” can span lines.
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!h3) continue;
    const headerName = stripBold(h3[1]).replace(/["â€œâ€]/g, '').trim();
  ...
  for (const b of collectBullets(lines)) {
    const mm = b.match(/^\*\*([^*]+?)\*\*\s*(.+)$/);
    if (!mm) continue;
    const nameRaw = mm[1].replace(/[.:]\s*$/, '').replace(/["â€œâ€]/g, '').trim();
...
function parseColorBullet(bullet) {
  ...
  // Case 2 (Stitch): **Name (values):** description   â€” value embedded in bold.
...
function extractTypography(section) {
  ...
  // Character paragraph â€” either a **Character:** label, or fall back to the
...
function normalizeFontRole(raw) {
  ...
  // â€” collapse them to the first canonical role present.
