...
export function resolveContextDir(cwd = process.cwd()) {
  ...
  // 4. Nothing found â€” keep the historical "default to cwd" behaviour so the
...
export function loadContext(cwd = process.cwd()) {
  ...
  //    it in place. We only migrate at the root â€” fallback dirs are read-only
  ...
  if (!productPath && contextDir === cwd) {
    ...
    if (legacyPath) {
      ...
      try {
        ...
        // Rename failed (permissions, etc.) â€” fall back to reading legacy in place
...
// ---------------------------------------------------------------------------
// CLI mode â€” print the context as JSON
// ---------------------------------------------------------------------------
