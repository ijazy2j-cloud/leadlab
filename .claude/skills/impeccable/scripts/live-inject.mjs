 ...
 * code. The user cannot turn these off via config â€” they are the floor.
 */
const HARD_EXCLUDES = [
  '**/node_modules/**',
  '**/.git/**',
...
export function resolveFiles(rootDir, config) {
  ...
  for (const pat of patterns) {
    if (!isGlob(pat)) {
      // Literal path â€” include even if it doesn't exist yet; the caller
...
function globToRegex(pattern) {
  ...
  while (i < pattern.length) {
    ...
    if (c === '*') {
      if (pattern[i + 1] === '*') {
        // ** â€” any number of segments, including zero. Handle the common
...
function insertTag(content, config, port) {
  ...
  // insertAfter: match the FIRST occurrence â€” typical anchors like `<head>` or
...
// shared helpers) is NOT patched here â€” those need framework-specific config
...
function appendOriginToDirective(csp, directive, origin) {
  ...
  // Directive missing â€” add it. Use 'self' + origin so we don't inadvertently
...
export function patchCspMeta(content, port) {
  ...
  for (let i = tags.length - 1; i >= 0; i--) {
    ...
    // URL.createObjectURL, producing a `blob:` URL â€” img-src 'self' rejects
    ...
    // `<meta â€¦ />` round-trips byte-for-byte.
