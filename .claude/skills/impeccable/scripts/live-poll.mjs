...
Options:
  ...
  if (replyIdx !== -1) {
    ...
    try {
      ...
      // Success â€” silent exit (agent doesn't need output for replies)
    } catch (err) {
  ...
  try {
    ...
    // JSON but skips nested fields. One line is enough â€” the full checklist
    // is in reference/live.md.
    if (event._acceptResult?.carbonize === true) {
      process.stderr.write('\nâš  Carbonize cleanup REQUIRED before next poll. After cleanup, run live-complete.mjs --id ' + event.id + '. See reference/live.md "Required after accept".\n\n');
    }

    // Print the event as JSON â€” the agent reads this from stdout
