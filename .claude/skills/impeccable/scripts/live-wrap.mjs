...
The agent should insert variant HTML at insertLine.`);
  ...
  if (text) {
    ...
    for (const q of queries) {
      ...
      // result, stop â€” falling through to the loose tag+single-class query
    ...
    if (candidates.length === 1) {
      ...
      if (filtered.length === 1) {
        ...
        // back to first-match rather than refusing â€” this is the same
        // behavior unmodified callers see, just preserved.
        match = candidates[0];
      } else {
        // Multiple candidates ALSO match the text. Truly ambiguous â€” refuse
  ...
  // the relative depth between lines â€” `l.trimStart()` would strip ALL leading
  ...
  // comment` yields three adjacent siblings â€” invalid JSX. We can't use a
...
function findElement(lines, query, tag = null) {
  // Iterate all matches â€” the first substring hit isn't always the right one.
 ...
 * inserting whitespace (e.g. `<h1>Hero Two</h1><p>Secondâ€¦</p>` reads as
 * `"Hero TwoSecondâ€¦"`), while the source has whitespace between tags. If
 * EITHER normalization matches, the candidate keeps. A snippet shorter than
 * 8 chars after stripping is too weak to disambiguate â€” the caller falls
 * back to first-match.
 */
function filterByText(candidates, lines, text) {
  ...
  // === 0` branch fires (fall back to first-match) â€” the previous
...
function findOpenerLine(lines, matchLine, tag) {
  ...
  for (let i = matchLine - 1; i >= Math.max(0, matchLine - MAX_BACKWALK); i--) {
    ...
    // Different tag name than requested â€” abort; we're inside a non-target opener.
    return -1;
  }
  return -1;
}
