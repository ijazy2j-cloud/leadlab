...
// reference â€” the computed style for the element then shows empty width,
...
//     descendants are not tracked â€” uncommon in practice and would require
...
// recognise is passed through unchanged â€” isNeutralColor then treats it as
  ...
  for (const sheet of document.styleSheets) {
    ...
    for (const rule of rules) {
      ...
      for (const [prop, side] of [
        ...
        // Width may or may not come from this rule â€” that's fine; the
      ...
      for (const el of matched) {
        ...
        if (existing) {
          // Later rules overwrite earlier ones â€” approximates source-order
...
// Strip `@layer NAME { â€¦ }` wrappers from a CSS / HTML source, leaving
...
function unwrapCssAtLayer(source) {
  ...
  while ((m = re.exec(source)) !== null) {
    ...
    if (depth !== 0) {
      // Unbalanced â€” bail and return source unchanged.
      return source;
    }
