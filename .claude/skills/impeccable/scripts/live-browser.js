/**
 * Impeccable Live Variant Mode â€” Browser Script
 *
 ...
 * before this code.
 *
 * UI: a single floating bar that morphs between three states â€”
...
(function () {
  ...
  // 1.5 stroke â€” visually consistent with the Foundation grid on the homepage.
  ...
  // Scroll lock â€” holds window.scrollY at a fixed value while the session is
  ...
  // Dedicated key for scroll position â€” SEPARATE from LS_KEY so that
  ...
  //
  // Click events still bubble normally â€” only the early pointer/focus
  ...
  const DRAG_THRESHOLD = 5;       // px â€” below this, treat pointerup as a click
  ...
  function hideAnnotOverlay() {
    ...
    // Drop any in-progress edit without touching annotState â€” clearAnnotations
  ...
  function onAnnotDown(e) {
    ...
    // 1) Clear chip â†’ wipe all annotations
    ...
    // 2) Stroke hit path â†’ delete that stroke
    ...
    // 3) Pin â†’ drag, edit, or delete-on-double-click
    const pinWrap = e.target.closest?.('[data-annot-pin]');
    if (pinWrap) {
      const idx = parseInt(pinWrap.dataset.annotPin, 10);
      if (!Number.isInteger(idx)) return;
      // Double-click (two pointerdowns on the same pin within window) â†’ delete.
      const now = Date.now();
      ...
      // input keep focus (don't start a drag â€” the click wasn't meant as one).
    ...
    // 4) Empty area â†’ commit any open edit, then start new annotation
    if (annotEditing) {
      finalizeEditingPin();
      e.stopPropagation(); e.preventDefault();
      return;
  ...
  // ---------------------------------------------------------------------------
  // The Bar â€” one floating element, three modes
  // ---------------------------------------------------------------------------
  ...
  function buildConfigureRow() {
    ...
    // Freeform input. Focus state shows an accent-colored border only â€”
  ...
  function buildCyclingRow() {
    ...
    // Tune chip â€” only when the visible variant exposes params
    ...
    // Accept â€” primary action, uses the site's saturated brand magenta
  ...
  function buildDots(clickable) {
    ...
    for (let i = 1; i <= expectedVariants; i++) {
      ...
      // dots â€” the previous "accent ring + ash fill" combo read as noisy
  ...
  //
  //   range  / numeric toggle  â†’ CSS var  (`--p-<id>`)  used via var(--p-foo, N)
  //   steps  / boolean toggle  â†’ data-p-<id> attribute  used via :scope[data-p-foo="..."]
  //
  ...
  let paramsCurrentValues = {}; // {paramId: value} â€” mirror of the visible variant's live values
  ...
  function initParamsPanel() {
    ...
    // with mask-size growing from 0% to 100% along the bar-facing axis â€” no
    ...
    Object.assign(paramsPanelEl.style, {
      ...
      // in the transition list, so they snap instantly â€” no fly-in from the
  ...
  function showVariantInDOM(sessionId, num) {
    ...
    // Unconditional refresh â€” covers first-reveal (no-op if state isn't
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    // Apply target synchronously, not via rAF â€” racing the browser's
  ...
  // ---------------------------------------------------------------------------
  // SSE (serverâ†’browser) + fetch POST (browserâ†’server)
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          break;
        case 'done':
          // Variants already arrived via HMR â†’ normal transition.
          if (arrivedVariants >= expectedVariants && expectedVariants > 0) {
            if (state === 'GENERATING') {
              state = 'CYCLING';
              updateBarContent('cycling');
          ...
          // that path with a toast â€” better than the prior force-reload
          // which reset framework state and left the session stuck.
          setTimeout(() => {
            if (arrivedVariants >= expectedVariants && expectedVariants > 0) return;
            if (state !== 'GENERATING') return;
            showToast(
              "Variants ready. If the picked element isn't visible, retrace the path that revealed it â€” they'll appear automatically.",
              15000,
            );
          }, 2000);
          break;
   ...
   * inside a container whose visibility is gated by ephemeral state â€” modals,
   ...
   *
   * Heuristic, intentionally narrow â€” only fires for unambiguous cases so
  ...
  function maybeWarnConditionalAncestor(el) {
    ...
    while (node && depth < 12) {
      ...
      // 3. Tab panel â€” only meaningful when the page also shows ANOTHER
  ...
  // Dedupe per session by pathname â€” clicking around on the same page doesn't
  // re-fire.
  //
  ...
  // a browser-side debounce (~800â€“1000ms, cancelled on Go) if we want to
  ...
  function handleGo() {
    ...
    // screenshot is uploaded (or capture fails â€” we still emit, just without
    // screenshotPath).
  ...
  // access, so modern-screenshot can't embed them on its own â€” the resulting
  ...
  // genuinely transparent (no own color, no own image) â€” in that case
  ...
  function resolveCanvasBackground(el) {
    ...
    // `getComputedStyle(body).backgroundColor || â€¦` chain is a trap: that
    ...
    // transparent-black â€” modern-screenshot then renders the capture on a
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    // Light up the shader overlay the moment capture is ready â€” no reason to
    ...
    // are present. Without annotations the image is pure visual anchoring â€”
  ...
  // ---------------------------------------------------------------------------
  // Shader overlay â€” renders the captured screenshot as a WebGL texture and
...
// Asymmetric roller band. Product of two one-sided smoothsteps â€” peaks at
...
void main() {
  ...
  // Darker cells â†’ bigger magenta dots (classic risograph halftone curve).
  ...
  async function showShaderOverlay(el, blob, rect) {
    ...
    if (!gl) {
      // WebGL unavailable â€” fall back to a plain <img> overlay so the user
  ...
  function handleAccept() {
    ...
    // replace the wrapper imminently. Don't eagerly replaceChild here â€” React
  ...
  function cleanup() {
    ...
    // mutate the DOM yet â€” HMR from the agent's source rewrite is on its way,
  ...
  function showToast(message, duration) {
    ...
    // the two never overlap. Read the bar's actual rect â€” its height varies
    // with hover-expanded labels â€” and fall back to a sensible default
  ...
  // ambient background and invert â€” dark bar on light pages, light bar on
  ...
  function detectPageTheme() {
    try {
      ...
      // Walk body â†’ html, taking the first opaque background. The browser's
      ...
      // dark. Honoring alpha avoids that â€” and falling through to <html>
      ...
      function readOpaque(el) {
        ...
        if (alpha < 0.5) return null; // transparent / nearly transparent â†’ skip
        return [+m[1], +m[2], +m[3]];
      }

      const rgb = readOpaque(document.body) || readOpaque(document.documentElement);
      // Both transparent â†’ fall back to the browser's effective canvas color.
  ...
  // Impeccable logo mark â€” matches the site-header SVG (rounded square + "/").
  ...
  function initGlobalBar() {
    ...
    // Brand mark â€” fills bar height on the left. Left side inherits the bar's
    ...
    function makeIconBtn({ id, svg, label, ariaLabel, labelFont, onClick }) {
      ...
      // thrashing â€” the whole bar grows once and shrinks once.
    ...
    // Pick toggle â€” starts active (primary intent when entering live mode).
    ...
    // DESIGN.md panel toggle â€” quartet of color squares as the mark.
    ...
    // Exit Ã— on the right â€” intentionally subtle (textDim at rest, text on
    ...
    // of the visible bar â€” the X stays invisible even though the styles in
  ...
  function updateGlobalBarState() {
    ...
    // If the bar is currently under the cursor, keep all labels expanded â€”
  ...
  // ---------------------------------------------------------------------------
  // Design System Panel â€” visualizes the project's .impeccable/design.json sidecar
  // ---------------------------------------------------------------------------
  ...
  function loadDesignPrefs() {
    // `open` is intentionally NOT persisted â€” the panel always starts closed
  ...
  // Neutral panel palette â€” deliberately NOT Impeccable-branded. The panel is
  ...
  function renderDesignChrome() {
    ...
    // (Panel toggle lives in the global bar â€” no floating FAB.)
    // Panel
  ...
  function renderDesignBody() {
    ...
    if (designState.loading) {
      body.appendChild(msgDiv('loading', 'Loading design systemâ€¦'));
      return;
    }
    ...
    // Visual tab â€” single unified render path.
  ...
  // Frontmatter primitives + sidecar colorMeta â†’ tile-ready color models.
  ...
  function renderTypeTiles(body, types) {
    for (const t of types) {
      ...
      specimen.style.fontSize = '56px';  // Fixed specimen size â€” compare faces, not scales.
  ...
  function renderComponentTiles(body, components) {
    ...
    for (const group of groups) {
      ...
      // skip â€” per-item descriptions clutter a grouped tile.
  ...
  function renderOverviewCollapsible(n) {
    ...
    if (n.northStar) {
      ...
      star.textContent = 'â€œ' + n.northStar + 'â€';
      ov.appendChild(star);
    }
    if (n.overview) {
  ...
  function inlineMd(text) {
    ...
    // Italic (only single *â€¦*, skip if inside bold already handled)
    s = s.replace(/(^|[^*])\*([^*\n]+)\*(?!\*)/g, '$1<em>$2</em>');
    return s;
  }
